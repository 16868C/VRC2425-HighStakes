...
void redGoalStake() {
	...
	chassis.turnAbsolute(-65_deg, 650, {});
	chassis.moveToPoint({144_in, 4_in}, 1800, {.minRPM=600_rpm});
...
void redRingStake() {
	...
	chassis.turnAbsolute(350_deg, 1500, {.gains={1.1, 0.2, 0.1}, .errorMargin=3_deg, .turnWheel=TurnWheel::RIGHT});
	...
	chassis.moveToPoint({4_in, 4_in}, 1700, {});
	...
	chassis.turnAbsolute(70_deg, 0, {.errorMargin=5_deg, .turnWheel=TurnWheel::LEFT});
	chassis.moveDistance(15_in, 70_deg, 0, {.minRPM=100_rpm});
	arm.move(4000);
	pros::delay(500);
	arm.move(0);
}
void blueRingStake() {
	...
	chassis.turnAbsolute(-170_deg, 1500, {.gains={1.1, 0.2, 0.1}, .errorMargin=3_deg, .turnWheel=TurnWheel::LEFT});
	...
	intake.hold();
	chassis.moveToPoint({144_in, 4_in}, 1700, {});
	...
	chassis.turnAbsolute(120_deg, 850, {.errorMargin=5_deg, .turnWheel=TurnWheel::RIGHT});
	chassis.moveDistance(15_in, 120_deg, 650, {.minRPM=100_rpm});
	arm.move(4000);
	pros::delay(500);
	arm.move(0);
}
...
void redGoalRush() {
	...
	chassis.moveToPoint({144_in, 4_in}, 2000, {.minRPM=400_rpm, .earlyExitRadius=5_in});
	// chassis.moveDistance(20_in, -45_deg, 1000, {.minRPM=600_rpm, .headingGains={}});
	...
	chassis.moveDistance(10_in, -45_deg, 1000, {.minRPM=600_rpm});
...
void blueGoalRush() {
	odometry.init({29_in, 24_in, 113_deg});
	intake.setTargetRing(RingColour::BLUE);
	...
	chassis.moveToPoint({18_in, 56_in}, 0, {.minRPM=600_rpm});
	...
	chassis.moveToPoint({51_in, 37_in}, 0, {.maxRPM=400_rpm, .headingGains={0.8, 0, 0.02}, .reverse=true});
	clamp.extend();
	pros::delay(100);
	...
	chassis.moveToPoint({0_in, 4_in}, 2000, {.minRPM=400_rpm, .earlyExitRadius=5_in});
	// chassis.moveDistance(20_in, -45_deg, 1000, {.minRPM=600_rpm, .headingGains={}});
	chassis.moveDistance(-6_in, -135_deg, 600, {});
	intakeRaiser.retract();
	pros::Task([] {
		pros::delay(400);
		intakeRaiser.extend();
	});
	chassis.moveDistance(10_in, -135_deg, 1000, {.minRPM=600_rpm});
	
	// chassis.moveToPoint({103_in, 10_in}, 0, {.reverse=true});
	chassis.moveDistance(-40_in, -180_deg, 0, {});
	clamp.retract();
	chassis.turnAbsolute(-90_deg, 0, {.gains={1.2, 0.2, 0.09}, .turnWheel=TurnWheel::RIGHT});
	chassis.moveToPoint({38_in, 40_in}, 0, {.maxRPM=400_rpm, .reverse=true});
	clamp.extend();
	pros::delay(100);

	// chassis.turnAbsolute(-220_deg, 0, {.turnWheel=TurnWheel::LEFT});
	chassis.moveToPoint({70_in, 60_in}, 0, {});
	leftDoinker.extend();
	pros::delay(200);
	chassis.moveDistance(-25_in, 45_deg, 1200, {.maxRPM=300_rpm, .distGains={0.1, 0, 0.004}});
	leftDoinker.retract();
