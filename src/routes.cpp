...
void redRingAWPNoStake(){
	odometry.update({46_in, 21_in, -90_deg});//for blue side it is 
...
void redGoalAWP() {
	...
	chassis.moveToPoint({94_in, 52_in}, 0, {.maxRPM=400_rpm, .headingGains={0.4, 0, 0.01}, .reverse=true});
	...
	chassis.moveToPoint({126_in, 34_in}, 0, {.minRPM=200_rpm, .earlyExitRadius=2_in, .reverse=true});
	doinker.extend();
	claw.extend();
	// chassis.moveToPoint({138_in, 40_in}, 1150, {.reverse=true});
	chassis.turnAbsolute(-70_deg, 0, {.gains={1.25, 0.1, 0.12}, .turnWheel=TurnWheel::LEFT});
	chassis.moveToPoint({140_in, 23_in}, 1000, {});
	chassis.turnAbsolute(230_deg, 0, {.maxRPM=450_rpm, .minRPM=100_rpm, .gains={1.4, 0.1, 0.12}, .turnWheel=TurnWheel::RIGHT});
	chassis.turnAbsolute(145_deg, 0, {.gains={1.2, 0.2, 0.1}});
	doinker.retract();
	claw.retract();
	chassis.moveToPoint({100_in, 60_in}, 0, {});
}
void blueGoalAWP() {
...
void redGoalCorner() {
	...
	chassis.moveToPoint({110_in, 45_in}, 1300, {.reverse=true});//30_in
	intake.stop();
	chassis.turnAbsolute(225_deg, 1150, {});
	
	chassis.moveToPoint({125_in, 65_in}, 1250, {.maxRPM=400_rpm, .reverse=true});
	clamp.extend();
	intake.mogo();
	pros::delay(600);
	intake.intake();
	...
	clamp.retract();
	intake.stop();
	chassis.turnAbsolute(0_deg, 1100, {.gains={1.1, 0.1, 0.08}, .turnWheel=TurnWheel::RIGHT});
	chassis.moveToPoint({95_in, 55_in}, 1150, {.maxRPM=400_rpm, .reverse=true});
	clamp.extend();
	pros::delay(50);

	intakeRaiser.retract();
	chassis.turnAbsolute(230_deg, 1150, {.gains={1.2, 0.2, 0.1}});
	intake.mogo();
	chassis.moveDistance(24_in, 230_deg, 850, {});
	//chassis.moveToPoint({80_in, 39_in}, 0, {.maxRPM=300_rpm, .distGains={0.12, 0, 1.5}});
	pros::delay(300);
	intakeRaiser.extend();

	chassis.moveToPoint({127_in, 34_in}, 1150, {.minRPM=200_rpm, .earlyExitRadius=2_in, .reverse=true});
	doinker.extend();
	claw.extend();
	// chassis.moveToPoint({138_in, 40_in}, 1150, {.reverse=true});
	chassis.turnAbsolute(-20_deg, 1100, {.gains={1.1, 0.1, 0.08}, .turnWheel=TurnWheel::LEFT});
	chassis.moveToPoint({145_in, 23_in}, 1150, {});
	chassis.turnAbsolute(230_deg, 1100, {.maxRPM=450_rpm, .gains={1.1, 0.1, 0.08}, .turnWheel=TurnWheel::RIGHT});
